...
一个以离线为主的个人复习应用，把全部面试备考资料（总册、真题逐字稿、专项练习、考场贴士要点）结构化后装进手机。基于 HarmonyOS NEXT（Stage 模型，ArkTS + ArkUI），主打**底部 4 Tab 导航 + 真题套题模考 + 速记卡 + 个人学习档案 + 语音播报 + 开口自测转写 + 备考计划每日提醒**，随时随地复习。资料本地内置、检索在端侧完成；开口自测的「AI 智能评测」为可选联网能力，会将录音 / 转写文本发送至第三方语音与大模型服务。
...
| **速记卡** | 大卡片点按翻转看要点；**左滑**下一题、**右滑收藏当前卡**（流内完成，Toast 反馈）；底部「认识/不认识」标记掌握度并**持久化进度**（重开恢复到上次位置与已掌握集合）；卡片右上角星标显示收藏状态 |
